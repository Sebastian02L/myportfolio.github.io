...
function render(items) {
  items.forEach(item => {
    ...
    overlay.innerHTML = `<div class="portfolio-item-caption-content text-center text-white"><p class="lead">Haz click para ver más detalles sobre este proyecto</p></div>`;
            ...
            <div class="container">
              <div class="row justify-content-center">
                <div class="col-lg-8">
                  ...
                  <div class="videosYoutube mb-3"></div>
    ...
    if (item.type === 'video') {
      ...
      v.controls = true;
      v.preload = 'metadata'; // Añadido para no descargar automáticamente
      v.className = 'w-100';
      ...
      i.alt = item.title || '';
      i.loading = 'lazy'; // Añadido lazy loading
      media.appendChild(i);
    }
    ...
    if (Array.isArray(item.platforms) && item.platforms.length) {
      ...
      item.platforms.forEach(p => {
        ...
        img.alt = '';
        img.loading = 'lazy'; // Añadido lazy loading
    ...
    if (item.videos && item.videos.length) {
      ...
      item.videos.forEach(video => {
        ...
        videoEl.muted = true;
        videoEl.loop = true;
        videoEl.preload = 'metadata'; // Añadido para no descargar automáticamente
    ...
    //video youtube
    if (item.videoYoutube) {
      const videoYTEl = modal.querySelector('.videosYoutube');
      const h5 = document.createElement('h5');
      h5.textContent = 'Video del Proyecto:';
      videoYTEl.appendChild(h5);
      const videoEl = document.createElement('iframe');
      videoEl.src = item.videoYoutube.src;
      videoEl.width = item.videoYoutube.width || 560;
      videoEl.height = item.videoYoutube.height || 315;
      videoEl.frameBorder = 0;
      videoEl.setAttribute("allow", "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share");
      videoEl.setAttribute("referrerpolicy", "strict-origin-when-cross-origin");
      videoEl.setAttribute("allowfullscreen", "");
      videoYTEl.appendChild(videoEl);
    }
